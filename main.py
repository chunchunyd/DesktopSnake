...
def find_desktop_listview_handle():
    try:
        ...
        if not h_listview:
            print("错误: 找不到'SysListView32'窗口。")
            return None
        # print(f"成功找到桌面ListView句柄: {h_listview}")
        return h_listview
    except Exception as e:
        print(f"错误: 寻找句柄时发生异常: {e}")
        return None

def get_icon_count(h_listview):
    if not h_listview: return 0
    return win32gui.SendMessage(h_listview, commctrl.LVM_GETITEMCOUNT, 0, 0)

def get_icon_position(h_listview, index):
    if not h_listview: return None, None
    tid, pid = win32process.GetWindowThreadProcessId(h_listview)
    h_process = kernel32.OpenProcess(win32con.PROCESS_VM_OPERATION | win32con.PROCESS_VM_READ | win32con.PROCESS_VM_WRITE, False, pid)
    if not h_process: return None, None
    p_remote_buffer = None
    try:
        p_remote_buffer = kernel32.VirtualAllocEx(h_process, 0, ctypes.sizeof(POINT), win32con.MEM_COMMIT | win32con.MEM_RESERVE, win32con.PAGE_READWRITE)
        if not p_remote_buffer: return None, None
        ...
        kernel32.ReadProcessMemory(h_process, p_remote_buffer, ctypes.byref(local_point), ctypes.sizeof(local_point), ctypes.byref(bytes_read))
        return local_point.x, local_point.y
    finally:
        if p_remote_buffer: kernel32.VirtualFreeEx(h_process, p_remote_buffer, 0, win32con.MEM_RELEASE)
...
def restore_initial_positions(h_listview):
    global initial_positions
    if not initial_positions: return
    print("正在恢复图标初始位置...")
...
def stop_game():
    global game_running
    print("接收到停止信号，游戏即将退出...")
    game_running = False

def calculate_grid_parameters(h_listview):
    """根据图标在桌面上的实际位置计算网格参数。"""
    print("正在扫描桌面图标以计算网格参数...")
    all_icon_pos = []
    ...
    # left_column_icons = sorted([p for p in all_icon_pos if abs(p[0] - origin_x) < 20], key=lambda p: p[1])
    # if len(left_column_icons) < 2:
    #     print("错误：无法找到左下角边界图标。请确保在左上角图标的正下方放置一个图标作为边界。")
    #     return None
    ...
    if not pos1 or not pos2:
        print("错误：无法自动确定网格大小。")
        print("请确保在左上角图标的 正右方 和 正下方 紧邻的位置有其他图标。")
        return None
    ...
    grid_info = {
        "size_x": grid_size_x, "size_y": grid_size_y,
...
def set_dpi_awareness():
    ...
    try:
        ...
        try:
            ...
            print("Could not set DPI awareness (requires Windows Vista or later).")

def main():
    """主函数。"""
    global game_running

    print("提示：需要以管理员权限运行(因为用到了pywin32库)")
    print("按 ESC 可随时退出程序，恢复桌面图标位置。(如果使用 Ctrl+C 退出无法恢复桌面)")
    ...
    if icon_count < 5: 
        print(f"桌面图标太少({icon_count}个)，无法开始游戏。")
        return
    print(f"检测到 {icon_count} 个桌面图标。")

    print("请按以下步骤操作，然后按 Enter 键继续：")
    print("1. 在桌面上，右键 -> 查看 -> 勾选“将图标与网格对齐”。(win10/11默认即为勾选状态且不可更改)")
    print("2. 放置一个图标在屏幕的【左上角】。")
    print("3. 放置第二个图标在第一个图标【正右方】的紧邻位置。")
    print("4. 放置第三个图标在第一个图标【正下方】的紧邻位置。")
    print("5. 上述操作是为了计算网格大小。")
    input("完成后请按 Enter 键...")
    ...
    try:
        ...
        for i, icon_idx in enumerate(snake_indices):
            px, py = grid_to_pixel(snake_grid_pos[i][0], snake_grid_pos[i][1], grid_info)
            set_icon_position(h_desktop, icon_idx, px, py)
            time.sleep(0.01)
        ...
        if max_food_x < 5:
            print("错误：游戏区域太窄，无法安全放置食物。")
            restore_initial_positions(h_desktop)
            return

        food_grid_pos = (random.randint(5, max_food_x), random.randint(1, grid_info['rows'] - 2))
        while food_grid_pos in snake_grid_pos:
             food_grid_pos = (random.randint(0, max_food_x), random.randint(0, grid_info['rows'] - 1))
        ...
        while game_running:
            # --- 1. 键盘输入 ---
            # 这个检查会一直在循环中运行，保证了输入的及时响应
            new_direction = direction
            if keyboard.is_pressed('w') or keyboard.is_pressed('up'):
                if direction != (0, 1): new_direction = (0, -1)
            elif keyboard.is_pressed('s') or keyboard.is_pressed('down'):
                if direction != (0, -1): new_direction = (0, 1)
            elif keyboard.is_pressed('a') or keyboard.is_pressed('left'):
                if direction != (1, 0): new_direction = (-1, 0)
            ...
            if current_time - last_move_time >= game_speed:
                last_move_time = current_time # 重置计时器
                
                # --- 游戏逻辑只在节拍到达时执行 ---
                
                # --- 2.1 计算新蛇头位置 ---
                current_head_pos = snake_grid_pos[0]
                new_head_pos = (current_head_pos[0] + direction[0], current_head_pos[1] + direction[1])

                # --- 2.2 碰撞检测 ---
                # 撞墙
                if not (0 <= new_head_pos[0] < grid_info['cols'] and 0 <= new_head_pos[1] < grid_info['rows']):
                    print("游戏结束：撞到墙了！")
                    game_running = False
                    continue
                # 撞自己
                if new_head_pos in snake_grid_pos:
                    print("游戏结束：撞到自己了！")
                    game_running = False
                    continue
                # 撞边界
                if new_head_pos in border_grid_pos:
                    print("游戏结束：撞到边界图标了！")
                    game_running = False
                    continue
                ...
                if new_head_pos == food_grid_pos:
                    ...
                    if not waiting_icons:
                        print("恭喜你吃完了所有图标，游戏胜利！")
                        game_running = False
                        continue
                    
                    food_index = waiting_icons.pop(0)
                    if border_grid_pos: border_grid_pos.pop()
                    ...
                    while True:
                        food_grid_pos = (random.randint(0, max_food_x), random.randint(0, grid_info['rows'] - 1))
                        if food_grid_pos not in snake_grid_pos and food_grid_pos not in border_grid_pos:
                            break
                    ...
                    # --- 2.4 正常移动 ---
                    tail_grid_pos = snake_grid_pos.pop()
